...
np.random.seed(6666)
#load data
ratio = [1,2,3,4,6,7,8,9]
basedir = '/home/qty/yzn-workspace/spamDetection/data/YelpHotelResCombined/input/InfoGraph_Add_Loc01/'
graph_path=basedir+'graph.txt'
text_path=basedir+'data.txt'

for r in ratio:
	trainid_path = basedir + 'ColdStart_Update_twoDomain_r'+str(r)+'/trainEmb.txt'
	trainClaId_path_hotel = basedir + 'ColdStart_Update_hotel/train.txt'
	trainClaId_path_res = basedir + 'ColdStart_Update_twoDomain_r'+str(r)+'/trainEmb.txt'
	testid_hotel = basedir + 'ColdStart_Update_hotel/test.txt'
	testid_res = basedir + 'ColdStart_Update_restaurant/test.txt'
	pretrain_emb_path = basedir+"word2vec/review_shuffle_w2v_c1w8-i20h0n5s100.txt"
		...
		with sess.as_default():
			...
			for epoch in range(config.num_epoch):
				...
				for i in range(num_batch):
					...
					textnode = map(int,textnode)
					text_neg_node = map(int,text_neg_node)
			...
			embedpath = basedir +'emb/'
			if (not os.path.exists(embedpath)):
				os.mkdir(embedpath)
			file_flat=open(embedpath +'embed(pt_ft)_Epoch' + str(
				config.num_epoch) +'Batch'+str(config.batch_size) +'_Tflat('+trainmode+')_r'+str(r)+'.txt', 'wb')
			file_sim_ru = open(embedpath + 'embed(pt_ft)_Epoch' + str(
				config.num_epoch) + 'Batch' + str(config.batch_size) + '_TconcatSim(removeU)('+trainmode+')_r'+str(r)+'.txt', 'wb')
			file_sim_all = open(embedpath + 'embed(pt_ft)_Epoch' + str(
				config.num_epoch) + 'Batch' + str(
				config.batch_size) + '_TconcatAllAtt(removeU)(' + trainmode + ')_r'+str(r)+'.txt', 'wb')
